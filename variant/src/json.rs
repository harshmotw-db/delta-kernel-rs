...
impl<'a, T: MemoryAllocator> VariantBuilder<'a, T> {
    ...
    fn finish_writing_array(
        ...
        value_buffer[start + 1..offset_start].copy_from_slice(&num_offsets.to_le_bytes()[..size_bytes]);
...
mod tests {
    ...
    fn test_json_to_variant() -> Result<(), Box<dyn Error>> {
        ...
        // TODO: Decimals (including large integers)
        ...
        compare_results(
            &format!("[{} true]", std::iter::repeat("1, ").take(128).collect::<String>()),
            &[7u8, 129u8,
        ...
        compare_results(
            ...
            &[2u8, 3u8, 2u8, 1u8, 0u8, 13u8, 12u8, 0u8, 20u8,
            3u8, 3u8, 0u8, 2u8, 4u8, 6u8, 12u8, 4u8, 12u8, 3u8, 12u8, 1u8,
            0u8,
            3u8, 2u8, 0u8, 1u8, 2u8, 4u8, 8u8,]
        )?;
